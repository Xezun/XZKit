...
/// - SeeAlso: TimeTiker
public protocol TimeTickable: TimeTicking, TimeTickerDelegate {
    // TimeTickerDelegate 是唯一需要实现的方法
}

/// 定义了 TimeTicker 计时器所具有的属性和特征。
public protocol TimeTicking: AnyObject {
    ...
    /// - Note: 对于已启动的 TimeTicker 在再次 resume 前，改变此属性不会生效。
...
/// TimeTicker 代理协议。
public protocol TimeTickerDelegate: AnyObject {
    
    /// 计时器每执行一次嘀嗒计时，都会触发此方法。
    /// - Parameters:
    ///   - timeTicker: 调用此方法的 TimeTicker 计时器。
...
open class TimeTicker: TimeTicking {
    
    /// 代理。
    open weak var delegate: TimeTickerDelegate?
    ...
    private func dispatchTimerAction() {
        ...
        delegate?.timeTicker(self, didTick: delta)
    }
    
}

extension TimeTickable {
    
    public var duration: TimeInterval {
        get { return timeTicker.duration }
        set { timeTicker.duration = newValue }
    }
    
    public var timeInterval: TimeInterval {
        get { return timeTicker.timeInterval }
        set { timeTicker.timeInterval = newValue }
    }
    
    public var timeLeeway: DispatchTimeInterval {
        get { return timeTicker.timeLeeway }
        set { timeTicker.timeLeeway = newValue}
    }
    
    public var currentTime: TimeInterval {
        get { return timeTicker.currentTime }
        set { timeTicker.currentTime = currentTime }
    }
    ...
    /// - Note: 该属性可写，可自定义（比如使用其它队列）所使用的 TimeTicker 对象。
    public var timeTicker: TimeTicker {
        get {
            if let timeTicker = objc_getAssociatedObject(self, &AssociationKey.timeTicker) as? TimeTicker {
                return timeTicker
            }
            let timeTicker = TimeTicker.init(queue: .main)
