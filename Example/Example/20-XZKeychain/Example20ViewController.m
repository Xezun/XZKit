...
- (IBAction)searchButtonAction:(id)sender {
    ...
    if ([keychain search:YES error:&error]) {
        [self xz_showToast:[XZToast messageToast:@"读取成功"] duration:3.0 completion:nil];
        self.accountTextField.text = keychain.item.account;
        self.passwordTextField.text = keychain.item.password;
    } else {
        [self xz_showToast:[XZToast messageToast:@"读取失败"] duration:3.0 completion:nil];
...
- (IBAction)insertButtonAction:(id)sender {
    ...
    if (account == 0 || server == 0) {
        [self xz_showToast:[XZToast messageToast:@"帐号或密码不能为空"] duration:3.0 completion:nil];
        return;
    }
    XZKeychain<XZKeychainInternetPasswordItem *> *keychain = [XZKeychain keychainWithAccount:account password:nil server:server];
    ...
    if ([keychain insert:&error]) {
        [self xz_showToast:[XZToast messageToast:@"保存成功"] duration:3.0 completion:nil];
    } else {
        [self xz_showToast:[XZToast messageToast:@"保存失败"] duration:3.0 completion:nil];
        self.messageLabel.text = error.localizedDescription;
    }
}
...
- (IBAction)deleteButtonAction:(id)sender {
    ...
    if ([keychain delete:&error]) {
        [self xz_showToast:[XZToast messageToast:@"删除成功"] duration:3.0 completion:nil];
        
    } else {
        [self xz_showToast:[XZToast messageToast:@"删除失败"] duration:3.0 completion:nil];
        self.messageLabel.text = error.localizedDescription;
    }
}
...
- (IBAction)updateButtonAction:(UIButton *)sender {
    ...
    if ([keychain search:NO error:&error]) {
        ...
        if ([keychain update:&error]) {
            [self xz_showToast:[XZToast messageToast:@"修改成功"] duration:3.0 completion:nil];
        } else {
            [self xz_showToast:[XZToast messageToast:@"修改失败"] duration:3.0 completion:nil];
            self.messageLabel.text = error.localizedDescription;
        }
    } else {
        [self xz_showToast:[XZToast messageToast:@"没有找到钥匙串"] duration:3.0 completion:nil];
        self.messageLabel.text = error.localizedDescription;
    }
}
