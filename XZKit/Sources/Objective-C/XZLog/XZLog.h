...
FOUNDATION_EXTERN void __XZLog__(const char * const filePath, int const line, const char * const function, NSString * const _Nonnull format, ...) NS_FORMAT_FUNCTION(4, 5) XZ_OBJC_ONLY;

/// 控制台输出。
/// @note 仅在程序添加了启动参数 -XZKitDEBUG 才执行控制台输出。
/// @note 仅在 DEBUG 模式输出请用 DZLOG 宏。
FOUNDATION_EXTERN void XZLog(NSString * _Nonnull format, ...) XZ_OBJC_ONLY;
#undef  XZLog
...
/// @note 仅在程序添加了启动参数 -XZKitDEBUG 才执行控制台输出。
/// @note 仅在 DEBUG 模式输出。
FOUNDATION_EXTERN void DLOG(NSString * _Nonnull format, ...) XZ_OBJC_ONLY;
