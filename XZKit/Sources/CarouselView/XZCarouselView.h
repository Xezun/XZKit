...
/// 轮播状态。
typedef NS_ENUM(NSInteger, XZCarouselViewDirection) {
    /// 正向轮播。
    XZCarouselViewDirectionForward  = 1,
    /// 反向轮播。
    XZCarouselViewDirectionBackward = -1
} NS_SWIFT_NAME(CarouselView.Direction);

/// 轮播方向。
typedef NS_ENUM(NSInteger, XZCarouselViewOrientation) {
    /// 水平轮播。
    XZCarouselViewOrientationHorizontal = 0,
    /// 垂直轮播。
    XZCarouselViewOrientationVertical
} NS_SWIFT_NAME(CarouselView.Orientation);
...
/// 轮播方向。
/// @note 为了避免使用 IB_DESIGNABLE/IBInspectable 标识，在 xib 或 storyboard 中，设置 tag 为 -1 表示视图为垂直方向轮播。
@property (nonatomic) XZCarouselViewOrientation orientation;

- (instancetype)initWithFrame:(CGRect)frame;
- (instancetype)initWithFrame:(CGRect)frame orientation:(XZCarouselViewOrientation)orientation NS_DESIGNATED_INITIALIZER;
...
/// 数据源代理。
/// @note 更新数据源后，必须调用 reloadData() 或 setCurrentIndex(_:animated:) 方法。
...
@property (nonatomic) XZCarouselViewDirection direction;
...
/// 当转场开始时，此方法会被调用。一般情况下，此方法用于处理 transitioningView 的转场动画。
///
...
/// 当转场结束时，此方法会被调用。一般情况下，此方法用于执行移除转场动画的操作。
