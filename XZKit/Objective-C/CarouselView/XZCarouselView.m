...
static XZCarouselViewDirection XZCarouselViewPagingDirectionMake(BOOL const isWrapped, NSInteger const fromIndex, NSInteger const toIndex, NSInteger const count);
...
- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame orientation:(XZCarouselViewOrientationHorizontal)];
}

- (instancetype)initWithFrame:(CGRect)frame orientation:(XZCarouselViewOrientation)pagingOrientation {
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        XZCarouselViewOrientation const pagingOrientation = (super.tag == -1 ? XZCarouselViewOrientationVertical : XZCarouselViewOrientationHorizontal);
...
- (void)_XZCarouselViewDidInitialize:(XZCarouselViewOrientation const)pagingOrientation contentMode:(UIViewContentMode const)contentMode {
    _isWrapped             = YES; // 默认无限轮播模式开启。
    _currentIndex          = XZCarouselViewNotFound; // 默认没有内容。
    _numberOfViews         = 0;
    _direction       = XZCarouselViewDirectionForward; // 默认正向轮播。
...
- (void)setCurrentIndex:(NSInteger const)newIndex animated:(BOOL)animated {
    ...
    if (animated && !_isTransitioning && transition == 0) {
        ...
        XZCarouselViewDirection const scrollDirection = XZCarouselViewPagingDirectionMake(_isWrapped, _currentIndex, newIndex, _numberOfViews);
        _XZCarouselViewItemView * __unsafe_unretained targetItemView = nil;
        switch (scrollDirection) {
            case XZCarouselViewDirectionForward:
                if (_isWrapped) {
...
- (void)_XZCarouselViewScrollTimerAction:(NSTimer *)timer {
    ...
    if (!_isWrapped) {
        ...
        if (_currentIndex == 0) {
            _direction = XZCarouselViewDirectionForward;
        } else if (_currentIndex == _numberOfViews - 1) {
            _direction = XZCarouselViewDirectionBackward;
        }
    }
    // 添加转场动画。
    XZCarouselViewAddTransitionAnimationIfNeeded(self, NO);
    // 跳转。
    if (_direction == XZCarouselViewDirectionForward) {
...
- (XZCarouselViewOrientation)orientation {
    return _scrollView.pagingOrientation;
}

- (void)setOrientation:(XZCarouselViewOrientation)pagingOrientation {
    [_scrollView setDelegate:nil];
...
static XZCarouselViewDirection XZCarouselViewPagingDirectionMake(BOOL const isWrapped, NSInteger const fromIndex, NSInteger const toIndex, NSInteger const count) {
    if (isWrapped) {
        if (toIndex == 0 && fromIndex == count - 1) {
            return XZCarouselViewDirectionForward;
        }
        if (fromIndex == 0 && toIndex == count - 1) {
            return XZCarouselViewDirectionBackward;
        }
    }
    return (fromIndex < toIndex ? XZCarouselViewDirectionForward : XZCarouselViewDirectionBackward);
}
