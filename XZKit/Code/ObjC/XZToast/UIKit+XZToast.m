...
- (void)xz_showToast:(XZToast *)toast duration:(NSTimeInterval)duration position:(NSDirectionalRectEdge)position offset:(CGFloat)offset isExclusive:(BOOL)isExclusive completion:(void (^)(BOOL))completion {
    [[XZToastManager managerForViewController:self] showToast:toast duration:duration position:position offset:offset isExclusive:isExclusive completion:completion];
}
...
- (void)showToast:(XZToast *)toast duration:(NSTimeInterval)duration position:(NSDirectionalRectEdge)position offset:(CGFloat)offset isExclusive:(BOOL)isExclusive completion:(void (^ _Nullable)(BOOL))completion {
    ...
    item.task = dispatch_block_create(DISPATCH_BLOCK_NO_QOS_CLASS, ^{
        [self hideToast:item];
    });
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(duration * NSEC_PER_SEC)), dispatch_get_main_queue(), item.task);
...
- (void)updateToastsIfNeeded {
    ...
    // offset 为 _containerView 与 window 或 vc.view 的距离
    // _containerView 与 toastView 之间有 10 的边距，用来展示阴影
    // toastView 之间有 10 点的距离。
    CGFloat const padding = 20.0;
    CGFloat const spacing = 10.0;
