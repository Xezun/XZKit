...
extension Data {
    ...
    public init?(hexadecimalEncoded string: String) {
        ...
        while index <= maxIndex {
            guard let bit1 = UInt8.init(hexadecimal: string[index]) else {
                break
            }
            index = string.index(after: index);
            guard let bit2 = UInt8.init(hexadecimal: string[index]) else {
                break
            }
...
extension UInt8 {
    ...
    public init?(hexadecimal character: Character) {
        switch character {
        case "0": self = 0
        case "1": self = 1
        case "2": self = 2
