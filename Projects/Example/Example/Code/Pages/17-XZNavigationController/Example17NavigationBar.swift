...
// 示例功能
// 1、如何定制化导航栏
// 2、如何使用通过协议拓展的方法，让控制器遵循协议即可自动获得一个默认的定制化导航栏。
...
extension XZNavigationBarCustomizable {
    ...
    public var xzNavigationBar: XZNavigationBarProtocol? {
        ...
        // 但是，既然已经声明了，似乎没有必要如此考虑，因为不使用定制化导航栏的话，可以不必声明遵循协议。
        return self.navigationBar
    }
    
    // 将定制化导航栏提供给遵循协议的控制器使用。
    // 注意，控制拿到的是明确类型的定制化导航栏，而不是协议中的 AnyNavigationBar 类型。
    // 这可以帮我们在使用定制化导航栏时，省去类型转换的麻烦。
