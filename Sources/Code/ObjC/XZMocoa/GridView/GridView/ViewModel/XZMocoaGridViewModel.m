...
@interface XZMocoaGridViewModel () {
    ...
    NSOrderedSet<XZMocoaGridSectionViewModel *> *_dataBeforeBatchUpdates;
    /// 批量更新时，被延迟的更新。
    ...
    /// 保存所有 section 视图模型。
    NSMutableOrderedSet<XZMocoaGridSectionViewModel *> *_sectionViewModels;
}
...
- (NSArray<XZMocoaGridSectionViewModel *> *)sectionViewModels {
...
- (__kindof XZMocoaGridSectionViewModel *)sectionViewModelAtIndex:(NSInteger)index {
    return _sectionViewModels[index];
}

- (__kindof XZMocoaGridCellViewModel *)cellViewModelAtIndexPath:(NSIndexPath *)indexPath {
    return [_sectionViewModels[indexPath.section] cellViewModelAtIndex:indexPath.row];
}

- (NSInteger)indexOfSectionViewModel:(XZMocoaGridSectionViewModel *)sectionModel {
...
- (void)didReceiveUpdates:(XZMocoaUpdates *)updates {
    if ([updates.key isEqualToString:XZMocoaUpdatesKeyReload]) {
        ...
        if ([subViewModel isKindOfClass:[XZMocoaGridSectionViewModel class]]) {
            XZMocoaGridSectionViewModel * const sectionVM = subViewModel;
            NSInteger const section = [self indexOfSectionViewModel:sectionVM];
            if (section != NSNotFound) {
                if ([updates.source isKindOfClass:[XZMocoaGridCellViewModel class]]) {
...
- (void)reloadSectionsAtIndexes:(NSIndexSet *)sections {
    ...
    if (self.isPerformingBatchUpdates) {
        ...
        [sections enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
            XZMocoaGridSectionViewModel * const oldViewModel = [self sectionViewModelAtIndex:index];
        ...
        [sections enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
            ...
            id const newDataModel = [self model:model modelForSectionAtIndex:index];
            XZMocoaGridSectionViewModel *newViewModel = [self makeViewModelWithModel:newDataModel forSectionAtIndex:index];
            [self _insertSectionViewModel:newViewModel atIndex:index];
        }];
...
- (void)moveCellAtIndexPath:(NSIndexPath *)indexPath toIndexPath:(NSIndexPath *)newIndexPath {
    if (indexPath.section == newIndexPath.section) {
        ...
        XZMocoaGridSectionViewModel * const oldViewModel = [self sectionViewModelAtIndex:indexPath.section];
        XZMocoaGridCellViewModel * const viewModel = [oldViewModel removeCellViewModelAtIndex:indexPath.item];
        
        XZMocoaGridSectionViewModel * const newViewModel = [self sectionViewModelAtIndex:newIndexPath.section];
...
- (void)cleanupBatchUpdates {
    ...
    [_sectionViewModels enumerateObjectsUsingBlock:^(XZMocoaGridSectionViewModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        obj.index = idx;
    }];
}
...
- (NSIndexSet *)differenceBatchUpdatesIfNeeded {
    ...
    for (NSInteger i = 0; i < oldCount; i++) {
        XZMocoaGridSectionViewModel * const viewModel = oldViewModels[i];
    ...
    [deletes enumerateIndexesWithOptions:NSEnumerationReverse usingBlock:^(NSUInteger section, BOOL *stop) {
        XZMocoaGridSectionViewModel * const oldViewModel = [self sectionViewModelAtIndex:section];
        [oldViewModel removeFromSuperViewModel];
    }];
    ...
    [inserts enumerateIndexesUsingBlock:^(NSUInteger index, BOOL * _Nonnull stop) {
        ...
        XZMocoaGridSectionViewModel * const newViewModel = [self makeViewModelWithModel:newDataModel forSectionAtIndex:index];
        [self _insertSectionViewModel:newViewModel atIndex:index];
    ...
    for (NSInteger to = 0; to < newCount; to++) {
        if ([inserts containsIndex:to]) {
            ...
            XZMocoaGridSectionViewModel *viewModel = oldViewModels[to];
            ...
            XZMocoaGridSectionViewModel * const viewModel = oldViewModels[from];
            viewModel.model = newDataModels[to];
            NSInteger const index = [self indexOfSectionViewModel:viewModel];
            // 移动 section
            [self _moveSectionViewModelFromIndex:index toIndex:to];
...
- (void)_addSectionViewModel:(XZMocoaGridSectionViewModel *)sectionViewModel {
    NSParameterAssert([sectionViewModel isKindOfClass:[XZMocoaGridSectionViewModel class]]);
    [_sectionViewModels addObject:sectionViewModel];
    [self addSubViewModel:sectionViewModel];
}

/// 将 viewModel 插入到 index 位置，并添加为子元素。
- (void)_insertSectionViewModel:(XZMocoaGridSectionViewModel *)sectionViewModel atIndex:(NSInteger)index {
    NSParameterAssert([sectionViewModel isKindOfClass:[XZMocoaGridSectionViewModel class]]);
...
- (void)_loadDataWithoutEvents {
    ...
    for (NSInteger section = 0; section < count; section++) {
        ...
        XZMocoaGridSectionViewModel *viewModel = [self makeViewModelWithModel:dataModel forSectionAtIndex:section];
...
/// @param index 位置
- (XZMocoaGridSectionViewModel *)makeViewModelWithModel:(id)nullableModel forSectionAtIndex:(NSInteger)index {
    id<XZMocoaGridSectionModel> const model  = (nullableModel == (id)kCFNull ? nil : nullableModel);
    ...
    XZMocoaGridSectionViewModel * const viewModel = [[VMClass alloc] initWithModel:model];
...
@implementation XZMocoaGridViewModel (XZMocoaGridSectionViewModelUpdates)

- (void)sectionViewModel:(XZMocoaGridSectionViewModel *)viewModel didReloadData:(void * _Nullable)null {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
...
- (void)sectionViewModel:(XZMocoaGridSectionViewModel *)viewModel didReloadCellsAtIndexes:(NSIndexSet *)rows {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    if (!self.isReady) return;
...
- (void)sectionViewModel:(XZMocoaGridSectionViewModel *)viewModel didInsertCellsAtIndexes:(NSIndexSet *)rows {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    if (!self.isReady) return;
...
- (void)sectionViewModel:(XZMocoaGridSectionViewModel *)viewModel didDeleteCellsAtIndexes:(NSIndexSet *)rows {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    if (!self.isReady) return;
...
- (void)sectionViewModel:(XZMocoaGridSectionViewModel *)viewModel didMoveCellAtIndex:(NSInteger)row toIndex:(NSInteger)newRow {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    if (!self.isReady) return;
...
- (void)sectionViewModel:(XZMocoaGridSectionViewModel *)viewModel didPerformBatchUpdates:(void (^NS_NOESCAPE)(void))batchUpdates completion:(void (^ _Nullable)(BOOL))completion {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    ...
    switch (type) {
        ...
        case NSFetchedResultsChangeMove: {
            ...
            if ([anObject hasPersistentChangedValues]) {
                XZMocoaGridCellViewModel * const viewModel = [self cellViewModelAtIndexPath:indexPath];
