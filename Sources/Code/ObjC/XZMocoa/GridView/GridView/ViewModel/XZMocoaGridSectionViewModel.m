...
typedef void(^XZMocoaGridDelayedUpdates)(XZMocoaGridSectionViewModel *self);

@interface XZMocoaGridSectionViewModel () {
    ...
    /// 记录 cell 视图模型的数组。
    NSMutableOrderedSet<XZMocoaGridCellViewModel *> *_cellViewModels;
    NSMutableDictionary<XZMocoaKind, NSMutableArray<XZMocoaViewModel *> *> *_supplementaryViewModels;
}
@end

@implementation XZMocoaGridSectionViewModel
...
- (void)didReceiveUpdates:(XZMocoaUpdates *)updates {
    if ([updates.key isEqualToString:XZMocoaUpdatesKeyReload]) {
        ...
        if (self.isPerformingBatchUpdates) {
            [_delayedBatchUpdates addObject:^void(XZMocoaGridSectionViewModel *self) {
        ...
        for (NSString *key in _supplementaryViewModels) {
            for (XZMocoaGridSupplementaryViewModel *vm in _supplementaryViewModels[key]) {
                if (subViewModel == vm) {
                    [self didReloadData];
                    return;
                }
        ...
        // cell视图的更新事件
        if ([subViewModel isKindOfClass:[XZMocoaGridCellViewModel class]]) {
...
- (__kindof XZMocoaGridCellViewModel *)cellViewModelAtIndex:(NSInteger)index {
    return [_cellViewModels objectAtIndex:index];
}

- (NSInteger)indexOfCellViewModel:(XZMocoaGridCellViewModel *)cellModel {
...
- (void)reloadCellsAtIndexes:(NSIndexSet *)indexes {
    ...
    if (self.isPerformingBatchUpdates) {
        ...
        [indexes enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
            XZMocoaGridCellViewModel * const oldViewModel = [self cellViewModelAtIndex:index];
        ...
        [indexes enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
            XZMocoaGridCellViewModel * const oldViewModel = _cellViewModels[index];
...
- (NSIndexSet *)differenceBatchUpdatesIfNeeded {
    ...
    for (NSInteger i = 0; i < oldCount; i++) {
        XZMocoaGridCellViewModel * const viewModel = oldViewModels[i];
    ...
    [inserts enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
        id<XZMocoaGridCellModel>   const newDataModel = newDataModels[index];
        XZMocoaGridCellViewModel * const newViewModel = [self makeViewModelWithModel:newDataModel forCellAtIndex:index];
        [self insertCellViewModel:newViewModel atIndex:index];
    ...
    for (NSInteger to = 0; to < newCount; to++) {
        if ([inserts containsIndex:to]) {
            ...
            XZMocoaGridCellViewModel * const viewModel = oldViewModels[to];
            // 更新数据
            ...
            // 更新数据
            XZMocoaGridCellViewModel * const viewModel = oldViewModels[from];
            viewModel.model = newDataModels[to];
            // 移动位置
            NSInteger const index = [self indexOfCellViewModel:viewModel];
            [self _moveCellViewModelFromIndex:index toIndex:to];
...
- (void)_addCellViewModel:(XZMocoaGridCellViewModel *)cellViewModel {
    [_cellViewModels addObject:cellViewModel];
    [self addSubViewModel:cellViewModel];
}

- (XZMocoaGridCellViewModel *)removeCellViewModelAtIndex:(NSInteger)index {
    XZMocoaGridCellViewModel *viewModel = _cellViewModels[index];
    [_cellViewModels removeObjectAtIndex:index];
    [viewModel removeFromSuperViewModel];
    return viewModel;
}

- (void)insertCellViewModel:(XZMocoaGridCellViewModel *)cellViewModel atIndex:(NSInteger)index {
...
- (void)_loadDataWithoutEvents {
    ...
    for (XZMocoaKind kind in self.superViewModel.supportedSupplementaryKinds) {
        ...
        for (NSInteger index = 0; index < count; index++) {
            ...
            XZMocoaGridSupplementaryViewModel * const viewModel = [self makeViewModelWithModel:dataModel forSupplementaryElementOfKind:kind atIndex:index];
    ...
    for (NSInteger index = 0; index < count; index++) {
        id<XZMocoaGridCellModel> const dataModel = [self model:model modelForCellAtIndex:index];
        XZMocoaGridCellViewModel *viewModel = [self makeViewModelWithModel:dataModel forCellAtIndex:index];
...
- (XZMocoaGridCellViewModel *)makeViewModelWithModel:(id<XZMocoaGridCellModel> const)nullableModel forCellAtIndex:(NSInteger)index {
    ...
    XZMocoaGridCellViewModel *viewModel = [[VMClass alloc] initWithModel:model];
...
- (XZMocoaGridSupplementaryViewModel *)makeViewModelWithModel:(id const)nullableModel forSupplementaryElementOfKind:(XZMocoaKind)kind atIndex:(NSInteger)index {
    id<XZMocoaModel> const model  = (nullableModel == (id)kCFNull ? nil : nullableModel);
    XZMocoaName     const section = ((id<XZMocoaGridSectionModel>)self.model).mocoaName;
    XZMocoaName     const name    = model.mocoaName;
    ...
    XZMocoaGridSupplementaryViewModel *viewModel = [[VMClass alloc] initWithModel:model];
    viewModel.index      = index;
    viewModel.module     = module;
    viewModel.identifier = identifier;
    return viewModel;
...
@implementation XZMocoaGridSectionViewModel (XZMocoaGridSectionModelTransformer)

- (nullable id)model:(nullable id)model modelForCellAtIndex:(NSInteger)index {
    return [((id<XZMocoaGridSectionModel>)model) modelForCellAtIndex:index];
}

- (NSInteger)model:(nullable id)model numberOfCellModels:(void * _Nullable)null {
    return [((id<XZMocoaGridSectionModel>)model) numberOfCellModels];
}

- (nullable id)model:(nullable id)model modelForSupplementaryElementOfKind:(nonnull XZMocoaKind)kind atIndex:(NSInteger)index {
    return [((id<XZMocoaGridSectionModel>)model) modelForSupplementaryElementOfKind:kind atIndex:index];
}

- (NSInteger)model:(nullable id)model numberOfModelsForSupplementaryElementOfKind:(nonnull XZMocoaKind)kind {
    return [((id<XZMocoaGridSectionModel>)model) numberOfModelsForSupplementaryElementOfKind:kind];
}
