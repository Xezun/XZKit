//
//  XZMocoaTrellisViewSectionViewModel.m
//  XZMocoa
//
...
typedef void(^XZMocoaTrellisDelayedUpdates)(XZMocoaTrellisViewSectionViewModel *self);

@interface XZMocoaTrellisViewSectionViewModel () {
    ...
    /// 记录 cell 视图模型的数组。
    NSMutableOrderedSet<XZMocoaTrellisViewCellViewModel *> *_cellViewModels;
    NSMutableDictionary<XZMocoaKind, NSMutableArray<XZMocoaViewModel *> *> *_supplementaryViewModels;
}
@end

@implementation XZMocoaTrellisViewSectionViewModel
...
- (void)didReceiveUpdates:(XZMocoaUpdates *)updates {
    if ([updates.key isEqualToString:XZMocoaUpdatesKeyReload]) {
        ...
        if (self.isPerformingBatchUpdates) {
            [_delayedBatchUpdates addObject:^void(XZMocoaTrellisViewSectionViewModel *self) {
        ...
        for (NSString *key in _supplementaryViewModels) {
            for (XZMocoaTrellisViewSupplementaryViewModel *vm in _supplementaryViewModels[key]) {
                if (subViewModel == vm) {
                    [self didReloadData];
                    return;
                }
        ...
        // cell视图的更新事件
        if ([subViewModel isKindOfClass:[XZMocoaTrellisViewCellViewModel class]]) {
...
- (__kindof XZMocoaTrellisViewCellViewModel *)cellViewModelAtIndex:(NSInteger)index {
    return [_cellViewModels objectAtIndex:index];
}

- (NSInteger)indexOfCellViewModel:(XZMocoaTrellisViewCellViewModel *)cellModel {
...
- (void)reloadCellsAtIndexes:(NSIndexSet *)indexes {
    ...
    if (self.isPerformingBatchUpdates) {
        ...
        [indexes enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
            XZMocoaTrellisViewCellViewModel * const oldViewModel = [self cellViewModelAtIndex:index];
        ...
        [indexes enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
            XZMocoaTrellisViewCellViewModel * const oldViewModel = _cellViewModels[index];
...
- (void)cleanupBatchUpdates {
    ...
    for (XZMocoaTrellisDelayedUpdates batchUpdates in _delayedBatchUpdates) {
        batchUpdates(self);
    }
...
- (NSIndexSet *)differenceBatchUpdatesIfNeeded {
    ...
    for (NSInteger i = 0; i < oldCount; i++) {
        XZMocoaTrellisViewCellViewModel * const viewModel = oldViewModels[i];
    ...
    [inserts enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
        id<XZMocoaTrellisViewCellModel>   const newDataModel = newDataModels[index];
        XZMocoaTrellisViewCellViewModel * const newViewModel = [self makeViewModelWithModel:newDataModel forCellAtIndex:index];
        [self insertCellViewModel:newViewModel atIndex:index];
    ...
    for (NSInteger to = 0; to < newCount; to++) {
        if ([inserts containsIndex:to]) {
            ...
            XZMocoaTrellisViewCellViewModel * const viewModel = oldViewModels[to];
            // 更新数据
            ...
            // 更新数据
            XZMocoaTrellisViewCellViewModel * const viewModel = oldViewModels[from];
            viewModel.model = newDataModels[to];
            // 移动位置
            NSInteger const index = [self indexOfCellViewModel:viewModel];
            [self _moveCellViewModelFromIndex:index toIndex:to];
...
- (void)_addCellViewModel:(XZMocoaTrellisViewCellViewModel *)cellViewModel {
    [_cellViewModels addObject:cellViewModel];
    [self addSubViewModel:cellViewModel];
}

- (XZMocoaTrellisViewCellViewModel *)removeCellViewModelAtIndex:(NSInteger)index {
    XZMocoaTrellisViewCellViewModel *viewModel = _cellViewModels[index];
    [_cellViewModels removeObjectAtIndex:index];
    [viewModel removeFromSuperViewModel];
    return viewModel;
}

- (void)insertCellViewModel:(XZMocoaTrellisViewCellViewModel *)cellViewModel atIndex:(NSInteger)index {
...
- (void)_loadDataWithoutEvents {
    ...
    for (XZMocoaKind kind in self.superViewModel.supportedSupplementaryKinds) {
        ...
        for (NSInteger index = 0; index < count; index++) {
            ...
            XZMocoaTrellisViewSupplementaryViewModel * const viewModel = [self makeViewModelWithModel:dataModel forSupplementaryElementOfKind:kind atIndex:index];
    ...
    for (NSInteger index = 0; index < count; index++) {
        id<XZMocoaTrellisViewCellModel> const dataModel = [self model:model modelForCellAtIndex:index];
        XZMocoaTrellisViewCellViewModel *viewModel = [self makeViewModelWithModel:dataModel forCellAtIndex:index];
...
- (XZMocoaTrellisViewCellViewModel *)makeViewModelWithModel:(id<XZMocoaTrellisViewCellModel> const)nullableModel forCellAtIndex:(NSInteger)index {
    id<XZMocoaModel> const model   = (nullableModel == (id)kCFNull ? nil : nullableModel);
    XZMocoaName      const section = ((id<XZMocoaTrellisViewSectionModel>)self.model).mocoaName;
    ...
    XZMocoaTrellisViewCellViewModel *viewModel = [[VMClass alloc] initWithModel:model];
...
- (XZMocoaTrellisViewSupplementaryViewModel *)makeViewModelWithModel:(id const)nullableModel forSupplementaryElementOfKind:(XZMocoaKind)kind atIndex:(NSInteger)index {
    id<XZMocoaModel> const model  = (nullableModel == (id)kCFNull ? nil : nullableModel);
    XZMocoaName     const section = ((id<XZMocoaTrellisViewSectionModel>)self.model).mocoaName;
    XZMocoaName     const name    = model.mocoaName;
    ...
    XZMocoaTrellisViewSupplementaryViewModel *viewModel = [[VMClass alloc] initWithModel:model];
    viewModel.index      = index;
    viewModel.module     = module;
    viewModel.identifier = identifier;
    return viewModel;
...
@implementation XZMocoaTrellisViewSectionViewModel (XZMocoaTrellisViewSectionModelTransformer)

- (nullable id)model:(nullable id)model modelForCellAtIndex:(NSInteger)index {
    return [((id<XZMocoaTrellisViewSectionModel>)model) modelForCellAtIndex:index];
}

- (NSInteger)model:(nullable id)model numberOfCellModels:(void * _Nullable)null {
    return [((id<XZMocoaTrellisViewSectionModel>)model) numberOfCellModels];
}

- (nullable id)model:(nullable id)model modelForSupplementaryElementOfKind:(nonnull XZMocoaKind)kind atIndex:(NSInteger)index {
    return [((id<XZMocoaTrellisViewSectionModel>)model) modelForSupplementaryElementOfKind:kind atIndex:index];
}

- (NSInteger)model:(nullable id)model numberOfModelsForSupplementaryElementOfKind:(nonnull XZMocoaKind)kind {
    return [((id<XZMocoaTrellisViewSectionModel>)model) numberOfModelsForSupplementaryElementOfKind:kind];
}
