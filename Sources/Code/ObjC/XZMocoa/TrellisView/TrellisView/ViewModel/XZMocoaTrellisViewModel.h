...
#import <XZKit/XZMocoaTrellisViewModelDefines.h>
#else
#import "XZMocoaViewModel.h"
#import "XZMocoaTrellisViewSectionViewModel.h"
#import "XZMocoaTrellisViewCellViewModel.h"
#import "XZMocoaTrellisModel.h"
#import "XZMocoaTrellisViewModelDefines.h"
#endif

NS_ASSUME_NONNULL_BEGIN

NS_SWIFT_UI_ACTOR @protocol XZMocoaTrellisViewModelDelegate <XZMocoaViewModelDelegate>
@end

/// 列表容器视图的视图模型。
/// @attention 由于需要管理列表 Cell 子视图，因此需要设置 `module` 属性才能正常工作。
@interface XZMocoaTrellisViewModel : XZMocoaViewModel <XZMocoaTrellisViewModel>
...
@property (nonatomic, weak) id<XZMocoaTrellisViewModelDelegate> delegate;
...
@property (nonatomic, readonly) NSArray<__kindof XZMocoaTrellisViewSectionViewModel *> *sectionViewModels;

- (__kindof XZMocoaTrellisViewSectionViewModel *)sectionViewModelAtIndex:(NSInteger)index;
- (NSInteger)indexOfSectionViewModel:(__kindof XZMocoaTrellisViewSectionViewModel *)sectionViewModel;
...
@interface XZMocoaTrellisViewModel (XZMocoaTrellisViewSectionViewModelUpdates)
/// section 发送的 Section 重载事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaTrellisViewSectionViewModel *)viewModel didReloadData:(void * _Nullable)null;
/// section 发送的 Cell 重载事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaTrellisViewSectionViewModel *)viewModel didReloadCellsAtIndexes:(NSIndexSet *)rows;
/// section 发送的 Cell 插入事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaTrellisViewSectionViewModel *)viewModel didInsertCellsAtIndexes:(NSIndexSet *)rows;
/// section 发送的 Cell 删除事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaTrellisViewSectionViewModel *)viewModel didDeleteCellsAtIndexes:(NSIndexSet *)rows;
/// section 发送的 Cell 移动事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaTrellisViewSectionViewModel *)viewModel didMoveCellAtIndex:(NSInteger)row toIndex:(NSInteger)newRow;
/// section 发送的批量更新事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaTrellisViewSectionViewModel *)viewModel didPerformBatchUpdates:(void (^NS_NOESCAPE)(void))batchUpdates completion:(void (^ _Nullable)(BOOL))completion;
@end

@interface XZMocoaTrellisViewModel (XZMocoaTrellisModelTransformer)
...
@interface XZMocoaTrellisViewModel (NSFetchedResultsControllerDelegate) <NSFetchedResultsControllerDelegate>
