...
#import "XZMocoaTrellisViewModel.h"
...
typedef void(^XZMocoaTrellisDelayedUpdates)(__kindof XZMocoaViewModel *self);

@interface XZMocoaTrellisViewModel () {
    /// 记录了批量更新前的数据，如果不为空，则表示当前处于批量更新过程中。
    NSOrderedSet<XZMocoaTrellisViewSectionViewModel *> *_dataBeforeBatchUpdates;
    /// 批量更新时，被延迟的更新。
    NSMutableArray<XZMocoaTrellisDelayedUpdates> *_delayedBatchUpdates;
    /// 是否需要执行批量更新的差异分析。
    /// @note 在批量更新时，由于同一对象不能重复操作，因此任一独立更新操作被调用时，都会标记此值为NO，以关闭差异分析，避免重复操作。
    BOOL _needsDifferenceBatchUpdates;
    /// 保存所有 section 视图模型。
    NSMutableOrderedSet<XZMocoaTrellisViewSectionViewModel *> *_sectionViewModels;
}

@end

@implementation XZMocoaTrellisViewModel
...
- (NSArray<XZMocoaTrellisViewSectionViewModel *> *)sectionViewModels {
...
- (__kindof XZMocoaTrellisViewSectionViewModel *)sectionViewModelAtIndex:(NSInteger)index {
    return _sectionViewModels[index];
}

- (__kindof XZMocoaTrellisViewCellViewModel *)cellViewModelAtIndexPath:(NSIndexPath *)indexPath {
    return [_sectionViewModels[indexPath.section] cellViewModelAtIndex:indexPath.row];
}

- (NSInteger)indexOfSectionViewModel:(XZMocoaTrellisViewSectionViewModel *)sectionModel {
...
- (void)didReceiveUpdates:(XZMocoaUpdates *)updates {
    if ([updates.key isEqualToString:XZMocoaUpdatesKeyReload]) {
        ...
        if (self.isPerformingBatchUpdates) {
            ...
            [_delayedBatchUpdates addObject:^void(XZMocoaTrellisViewModel *self) {
                [self didReceiveUpdates:updates];
            }];
            return;
        }
        if ([subViewModel isKindOfClass:[XZMocoaTrellisViewSectionViewModel class]]) {
            XZMocoaTrellisViewSectionViewModel * const sectionVM = subViewModel;
            NSInteger const section = [self indexOfSectionViewModel:sectionVM];
            if (section != NSNotFound) {
                if ([updates.source isKindOfClass:[XZMocoaTrellisViewCellViewModel class]]) {
...
- (void)reloadSectionsAtIndexes:(NSIndexSet *)sections {
    ...
    if (self.isPerformingBatchUpdates) {
        ...
        [sections enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
            XZMocoaTrellisViewSectionViewModel * const oldViewModel = [self sectionViewModelAtIndex:index];
        ...
        [sections enumerateIndexesUsingBlock:^(NSUInteger const index, BOOL * _Nonnull stop) {
            ...
            id const newDataModel = [self model:model modelForSectionAtIndex:index];
            XZMocoaTrellisViewSectionViewModel *newViewModel = [self makeViewModelWithModel:newDataModel forSectionAtIndex:index];
            [self _insertSectionViewModel:newViewModel atIndex:index];
        }];
...
- (void)moveCellAtIndexPath:(NSIndexPath *)indexPath toIndexPath:(NSIndexPath *)newIndexPath {
    if (indexPath.section == newIndexPath.section) {
        ...
        XZMocoaTrellisViewSectionViewModel * const oldViewModel = [self sectionViewModelAtIndex:indexPath.section];
        XZMocoaTrellisViewCellViewModel * const viewModel = [oldViewModel removeCellViewModelAtIndex:indexPath.item];
        
        XZMocoaTrellisViewSectionViewModel * const newViewModel = [self sectionViewModelAtIndex:newIndexPath.section];
...
- (void)cleanupBatchUpdates {
    _dataBeforeBatchUpdates = nil;
    for (XZMocoaTrellisDelayedUpdates const batchUpdates in _delayedBatchUpdates) {
        batchUpdates(self);
    }
    ...
    [_sectionViewModels enumerateObjectsUsingBlock:^(XZMocoaTrellisViewSectionViewModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        obj.index = idx;
    }];
}
...
- (NSIndexSet *)differenceBatchUpdatesIfNeeded {
    ...
    for (NSInteger i = 0; i < oldCount; i++) {
        XZMocoaTrellisViewSectionViewModel * const viewModel = oldViewModels[i];
    ...
    [deletes enumerateIndexesWithOptions:NSEnumerationReverse usingBlock:^(NSUInteger section, BOOL *stop) {
        XZMocoaTrellisViewSectionViewModel * const oldViewModel = [self sectionViewModelAtIndex:section];
        [oldViewModel removeFromSuperViewModel];
    }];
    ...
    [inserts enumerateIndexesUsingBlock:^(NSUInteger index, BOOL * _Nonnull stop) {
        ...
        XZMocoaTrellisViewSectionViewModel * const newViewModel = [self makeViewModelWithModel:newDataModel forSectionAtIndex:index];
        [self _insertSectionViewModel:newViewModel atIndex:index];
    ...
    for (NSInteger to = 0; to < newCount; to++) {
        if ([inserts containsIndex:to]) {
            ...
            XZMocoaTrellisViewSectionViewModel *viewModel = oldViewModels[to];
            ...
            XZMocoaTrellisViewSectionViewModel * const viewModel = oldViewModels[from];
            viewModel.model = newDataModels[to];
            NSInteger const index = [self indexOfSectionViewModel:viewModel];
            // 移动 section
            [self _moveSectionViewModelFromIndex:index toIndex:to];
...
- (void)_addSectionViewModel:(XZMocoaTrellisViewSectionViewModel *)sectionViewModel {
    NSParameterAssert([sectionViewModel isKindOfClass:[XZMocoaTrellisViewSectionViewModel class]]);
    [_sectionViewModels addObject:sectionViewModel];
    [self addSubViewModel:sectionViewModel];
}

/// 将 viewModel 插入到 index 位置，并添加为子元素。
- (void)_insertSectionViewModel:(XZMocoaTrellisViewSectionViewModel *)sectionViewModel atIndex:(NSInteger)index {
    NSParameterAssert([sectionViewModel isKindOfClass:[XZMocoaTrellisViewSectionViewModel class]]);
...
- (void)_loadDataWithoutEvents {
    ...
    for (NSInteger section = 0; section < count; section++) {
        ...
        XZMocoaTrellisViewSectionViewModel *viewModel = [self makeViewModelWithModel:dataModel forSectionAtIndex:section];
...
/// @param index 位置
- (XZMocoaTrellisViewSectionViewModel *)makeViewModelWithModel:(id)nullableModel forSectionAtIndex:(NSInteger)index {
    id<XZMocoaTrellisViewSectionModel> const model  = (nullableModel == (id)kCFNull ? nil : nullableModel);
    ...
    XZMocoaTrellisViewSectionViewModel * const viewModel = [[VMClass alloc] initWithModel:model];
...
@implementation XZMocoaTrellisViewModel (XZMocoaTrellisViewSectionViewModelUpdates)

- (void)sectionViewModel:(XZMocoaTrellisViewSectionViewModel *)viewModel didReloadData:(void * _Nullable)null {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
...
- (void)sectionViewModel:(XZMocoaTrellisViewSectionViewModel *)viewModel didReloadCellsAtIndexes:(NSIndexSet *)rows {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    if (!self.isReady) return;
...
- (void)sectionViewModel:(XZMocoaTrellisViewSectionViewModel *)viewModel didInsertCellsAtIndexes:(NSIndexSet *)rows {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    if (!self.isReady) return;
...
- (void)sectionViewModel:(XZMocoaTrellisViewSectionViewModel *)viewModel didDeleteCellsAtIndexes:(NSIndexSet *)rows {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    if (!self.isReady) return;
...
- (void)sectionViewModel:(XZMocoaTrellisViewSectionViewModel *)viewModel didMoveCellAtIndex:(NSInteger)row toIndex:(NSInteger)newRow {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
    if (!self.isReady) return;
...
- (void)sectionViewModel:(XZMocoaTrellisViewSectionViewModel *)viewModel didPerformBatchUpdates:(void (^NS_NOESCAPE)(void))batchUpdates completion:(void (^ _Nullable)(BOOL))completion {
    NSInteger const index = [self indexOfSectionViewModel:viewModel];
    if (index == NSNotFound) return;
...
@implementation XZMocoaTrellisViewModel (XZMocoaTrellisModelTransformer)

- (NSInteger)model:(id)model numberOfSectionModels:(void *)null {
    return [(id<XZMocoaTrellisModel>)model numberOfSectionModels];
}

- (id)model:(id)model modelForSectionAtIndex:(NSInteger)index {
    return [(id<XZMocoaTrellisModel>)model modelForSectionAtIndex:index];
}

@end


@implementation XZMocoaTrellisViewModel (NSFetchedResultsControllerDelegate)
    ...
    switch (type) {
        ...
        case NSFetchedResultsChangeMove: {
            ...
            if ([anObject hasPersistentChangedValues]) {
                XZMocoaTrellisViewCellViewModel * const viewModel = [self cellViewModelAtIndexPath:indexPath];
