...
///
/// 组件提供了``XZNavigationBar``基类，我们可以继承它，也可以使用其它遵循了``XZNavigationBarProtocol``协议的视图控件。
///
/// ### 关于系统导航栏
///
...
extension XZNavigationBarProtocol {
    ...
    /// 将定制化导航栏的`isHidden`状态同步给原生导航栏。
    public func synchronizeHiddenAppearance() {
        guard let navigationBar = self.uiNavigationBar else { return }
        let selector = #selector(setter: UINavigationBar.isHidden)
        xz_objc_msgSendSuper_void(navigationBar, type(of: navigationBar), selector, self.isHidden)
    }
    
    /// 将定制化导航栏的`isTranslucent`状态同步给原生导航栏。
    public func synchronizeTranslucentAppearance() {
        guard let navigationBar = self.uiNavigationBar else { return }
        let selector = #selector(setter: UINavigationBar.isTranslucent)
        xz_objc_msgSendSuper_void(navigationBar, type(of: navigationBar), selector, self.isTranslucent)
    }
    
    /// 将定制化导航栏的`prefersLargeTitles`状态同步给原生导航栏。
    public func synchronizeLargeTitlesAppearance() {
        guard let navigationBar = self.uiNavigationBar else { return }
        let selector = #selector(setter: UINavigationBar.prefersLargeTitles)
        xz_objc_msgSendSuper_void(navigationBar, type(of: navigationBar), selector, self.prefersLargeTitles)
    }
