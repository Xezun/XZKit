...
public protocol APIManager: APINetworking {
    ...
    /// 当接口请求发生错误时，此方法会被调用。
    ///
    /// 属性 retryIfFailed 为 true 的请求在失败时，会通过此方法获取当前时间到下次重试之间的时间间隔，返回 nil 终止重试。
    ///
    /// 主动取消的任务，不会触发重试询问。
    ///
    /// 参数 error 的属性 numberOfRetries 表示当前已重试的次数（不包括第一次请求，值为 nil 时表示当前非重试问答模式）。
    ///
    /// 返回合适的时间间隔，以提高程序性能，比如随次数逐渐增加每次重试的时间间隔等。
    /// - Parameters:
    ///   - request: 接口请求对象。
    ///   - error: 接口请求过程中的错误对象。
    @discardableResult
...
fileprivate class APITaskManager<Manager: APIManager>: APIGroup, APITimerDelegate {
    ...
    public func deadlineTimerWasTimeout(_ apiTask: Task) {
        ...
        complete(removedTask, data: nil, error: APIError.timeOver)
    }
...
fileprivate protocol APITimerDelegate: AnyObject {
...
fileprivate struct AssociationKey {
    nonisolated(unsafe) static var manager: Int = 0
}
