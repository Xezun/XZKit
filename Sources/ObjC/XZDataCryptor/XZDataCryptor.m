...
static BOOL XZDataCryptorThrowsError(CCCryptorStatus status, NSError * _Nullable __autoreleasing *error);
/// 构造 CCCryptorRef 的方法。
...
+ (XZDataCryptor *)encryptorWithAlgorithm:(XZDataCryptorAlgorithm *)algorithm mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding {
    return [self cryptorWithOperation:XZDataCryptorOperationEncrypt algorithm:algorithm mode:mode padding:padding];
}

+ (XZDataCryptor *)decryptorWithAlgorithm:(XZDataCryptorAlgorithm *)algorithm mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding {
...
- (NSData *)finish:(NSError *__autoreleasing  _Nullable *)error {
...
+ (XZDataCryptor *)AESEncryptor:(NSString *)key vector:(NSString *)vector mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding {
    XZDataCryptorAlgorithm * const algorithm = [XZDataCryptorAlgorithm AES:key vector:vector];
    return [self encryptorWithAlgorithm:algorithm mode:mode padding:padding];
}

+ (XZDataCryptor *)AESEncryptor:(NSString *)key vector:(NSString *)vector {
    return [self AESEncryptor:key vector:vector mode:XZDataCryptorModeCBC padding:(XZDataCryptorPKCS7Padding)];
}

+ (XZDataCryptor *)AESDecryptor:(NSString *)key vector:(NSString *)vector mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding {
    XZDataCryptorAlgorithm * const algorithm = [XZDataCryptorAlgorithm AES:key vector:vector];
    return [self decryptorWithAlgorithm:algorithm mode:mode padding:padding];
}

+ (XZDataCryptor *)AESDecryptor:(NSString *)key vector:(NSString *)vector {
    return [self AESDecryptor:key vector:vector mode:XZDataCryptorModeCBC padding:(XZDataCryptorPKCS7Padding)];
}

+ (NSData *)AESEncrypt:(NSData *)data key:(NSString *)key vector:(NSString *)vector mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding error:(NSError * _Nullable __autoreleasing *)error {
    XZDataCryptorAlgorithm *algorithm = [XZDataCryptorAlgorithm AES:key vector:vector];
    return [self encrypt:data algorithm:algorithm mode:mode padding:padding error:error];
}

+ (NSData *)AESEncrypt:(NSData *)data key:(NSString *)key vector:(NSString *)vector error:(NSError * _Nullable __autoreleasing *)error {
    return [self AESEncrypt:data key:key vector:vector mode:(XZDataCryptorModeCBC) padding:(XZDataCryptorPKCS7Padding) error:error];
}

+ (NSData *)AESDecrypt:(NSData *)data key:(NSString *)key vector:(NSString *)vector mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding error:(NSError * _Nullable __autoreleasing *)error {
    XZDataCryptorAlgorithm *algorithm = [XZDataCryptorAlgorithm AES:key vector:vector];
    return [self decrypt:data algorithm:algorithm mode:mode padding:padding error:error];
}

+ (NSData *)AESDecrypt:(NSData *)data key:(NSString *)key vector:(NSString *)vector error:(NSError * _Nullable __autoreleasing *)error {
    return [self AESDecrypt:data key:key vector:vector mode:(XZDataCryptorModeCBC) padding:(XZDataCryptorPKCS7Padding) error:error];
}
...
+ (XZDataCryptor *)DESEncryptor:(NSString *)key vector:(NSString *)vector mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding {
    XZDataCryptorAlgorithm * const algorithm = [XZDataCryptorAlgorithm DES:key vector:vector];
    return [self encryptorWithAlgorithm:algorithm mode:mode padding:padding];
}

+ (XZDataCryptor *)DESEncryptor:(NSString *)key vector:(NSString *)vector {
    return [self DESEncryptor:key vector:vector mode:XZDataCryptorModeCBC padding:(XZDataCryptorPKCS7Padding)];
}

+ (XZDataCryptor *)DESDecryptor:(NSString *)key vector:(NSString *)vector mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding {
    XZDataCryptorAlgorithm * const algorithm = [XZDataCryptorAlgorithm DES:key vector:vector];
    return [self decryptorWithAlgorithm:algorithm mode:mode padding:padding];
}

+ (XZDataCryptor *)DESDecryptor:(NSString *)key vector:(NSString *)vector {
    return [self DESDecryptor:key vector:vector mode:XZDataCryptorModeCBC padding:(XZDataCryptorPKCS7Padding)];
}

+ (NSData *)DESEncrypt:(NSData *)data key:(NSString *)key vector:(NSString *)vector mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding error:(NSError * _Nullable __autoreleasing *)error {
    XZDataCryptorAlgorithm *algorithm = [XZDataCryptorAlgorithm DES:key vector:vector];
    return [self encrypt:data algorithm:algorithm mode:mode padding:padding error:error];
}

+ (NSData *)DESEncrypt:(NSData *)data key:(NSString *)key vector:(NSString *)vector error:(NSError * _Nullable __autoreleasing *)error {
    return [self DESEncrypt:data key:key vector:vector mode:(XZDataCryptorModeCBC) padding:(XZDataCryptorPKCS7Padding) error:error];
}

+ (NSData *)DESDecrypt:(NSData *)data key:(NSString *)key vector:(NSString *)vector mode:(XZDataCryptorMode)mode padding:(XZDataCryptorPadding)padding error:(NSError * _Nullable __autoreleasing *)error {
    XZDataCryptorAlgorithm *algorithm = [XZDataCryptorAlgorithm DES:key vector:vector];
    return [self decrypt:data algorithm:algorithm mode:mode padding:padding error:error];
}

+ (NSData *)DESDecrypt:(NSData *)data key:(NSString *)key vector:(NSString *)vector error:(NSError * _Nullable __autoreleasing *)error {
    return [self AESDecrypt:data key:key vector:vector mode:(XZDataCryptorModeCBC) padding:(XZDataCryptorPKCS7Padding) error:error];
}
...
static CCCryptorRef XZDataCryptorContextMake(XZDataCryptorAlgorithm *algorithm, XZDataCryptorOperation operation, XZDataCryptorMode mode, XZDataCryptorPadding padding, NSError * _Nullable __autoreleasing *error) {
    ...
    int         const rnd = (int)algorithm.numberOfRounds;
    ...
    XZDataCryptorThrowsError(status, error);
    return context;
}
...
static BOOL XZDataCryptorThrowsError(CCCryptorStatus status, NSError * _Nullable __autoreleasing * error) {
    ...
    NSDictionary * const userInfo = @{
        NSLocalizedDescriptionKey: NSStringFromCCCryptorStatus(status)
    };
