...
- (void)updateToastsIfNeeded {
    ...
    XZLog(@"[XZToast] 周期开始：_waitingToShowTasks = %lu, _waitingToHideTasks = %lu", _waitingToShowTasks.count, _waitingToHideTasks.count);
        ...
        if (newTask) {
            ...
            if (_maximumNumberOfToasts > 0) {
                while (_showingTasks.count >= _maximumNumberOfToasts) {
                    ...
                    XZLog(@"[XZToast] 消息复用 %@ => %@", showingTask, newTask);
                }
            }
    ...
    XZLog(@"[XZToast] 执行动画：toast = %@, hidingTasks = %@", newTask.toast, _hideingTasks);
    ...
    [UIView animateKeyframesWithDuration:XZToastAnimationDuration delay:0 options:options animations:^{
        ...
        XZLog(@"[XZToast] 动画完成：finished = %d", finished);
...
- (void)task:(XZToastTask *)newTask hideingTask:(NSArray<XZToastTask *> *)_hideingTasks didFinishAnimation:(BOOL)finished {
    ...
    for (NSInteger i = 0, j = 0; i < _hideingTasks.count; i++) {
        ...
        while (j < _waitingToShowTasks.count) {
            ...
            hideingTask.toast->_view = nil;
            hideingTask.isViewReused = YES;
            XZLog(@"[XZToast] 消息复用 %@ => %@", hideingTask, waitingTask);
            break;
        }
    }
