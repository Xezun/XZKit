...
@property (nonatomic, readonly) NSArray<__kindof XZMocoaGridSectionViewModel *> *sectionViewModels;

- (__kindof XZMocoaGridSectionViewModel *)sectionViewModelAtIndex:(NSInteger)index;
- (NSInteger)indexOfSectionViewModel:(__kindof XZMocoaGridSectionViewModel *)sectionViewModel;
...
@interface XZMocoaGridViewModel (XZMocoaGridSectionViewModelUpdates)
/// section 发送的 Section 重载事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaGridSectionViewModel *)viewModel didReloadData:(void * _Nullable)null;
/// section 发送的 Cell 重载事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaGridSectionViewModel *)viewModel didReloadCellsAtIndexes:(NSIndexSet *)rows;
/// section 发送的 Cell 插入事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaGridSectionViewModel *)viewModel didInsertCellsAtIndexes:(NSIndexSet *)rows;
/// section 发送的 Cell 删除事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaGridSectionViewModel *)viewModel didDeleteCellsAtIndexes:(NSIndexSet *)rows;
/// section 发送的 Cell 移动事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaGridSectionViewModel *)viewModel didMoveCellAtIndex:(NSInteger)row toIndex:(NSInteger)newRow;
/// section 发送的批量更新事件，以刷新视图。
- (void)sectionViewModel:(__kindof XZMocoaGridSectionViewModel *)viewModel didPerformBatchUpdates:(void (^NS_NOESCAPE)(void))batchUpdates completion:(void (^ _Nullable)(BOOL))completion;
@end
