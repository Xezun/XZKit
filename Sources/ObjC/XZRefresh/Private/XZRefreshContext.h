...
typedef NS_ENUM(NSUInteger, XZRefreshState) {
    ...
    /// 掩码。包含此掩码的 XZRefreshState 状态，表明已将 refreshHeight 合并到 .contentInset 中。
    XZRefreshStateContentInsetsMask = (1 << 8),
    /// 掩码。包含此掩码的 XZRefreshState 状态，表明视图 UIScrollView 正在刷新中。
    XZRefreshStateRefreshingMask    = (1 << 9),
    ...
    XZRefreshStateRefreshingDelayedEvents   = (1 << 3) | XZRefreshStateRefreshingMask | XZRefreshStateContentInsetsMask,
    /// 正在刷新，已经发送事件给代理。
    XZRefreshStateRefreshing                = (1 << 4) | XZRefreshStateRefreshingMask | XZRefreshStateContentInsetsMask,
    /// 将要进入复原过程。已经退出刷新，但是仍在拖拽中，尚未恢复 UIScrollView 的 contentInsets 属性，需要在 willEndDragging 再执行恢复操作。
    XZRefreshStateWillRecovering            = (1 << 5) | XZRefreshStateContentInsetsMask,
