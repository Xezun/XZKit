...
/// 使用`-isEqual:`判断数组是否包含重复元素。
@property (nonatomic, readonly) BOOL xz_containsEqualObjects NS_SWIFT_NAME(containsEqualObjects);
...
/// 遍历并将结果返回结果生成新的数组。
///
/// 生成的新数组元素个数与原数组相同，且与原数组元素是一一对应的。
///
...
/// 查找第一个符合条件的对象。正向遍历。如果没有找到，返回 NSNotFound 值。
/// @param predicate 判断是否包含元素的块函数
...
/// 查找最后一个符合条件的对象。反向遍历。如果没有找到，返回 NSNotFound 值。
/// @param predicate 判断是否包含元素的块函数
- (NSInteger)xz_lastIndex:(BOOL (^NS_NOESCAPE)(ObjectType obj))predicate OBJC_SWIFT_UNAVAILABLE("请直接使用 Swift 版本");
...
/// @param changes 元素位置`index`发生变化的元素，`key`为在@b当前数组@c中的`index`，`value`为在@b原始数组@c中的index
/// @param remains 元素位置`index`保持不变的元素
...
/// 移除数组中最后一个元素，并返回该元素。如果数组中没有元素，则返回 nil 值。
- (nullable ObjectType)xz_removeLastObject OBJC_SWIFT_UNAVAILABLE("请直接使用 Swift 版本");

/// 移除数组中第一个元素，并返回该元素。如果数组中没有元素，则返回 nil 值。
- (nullable ObjectType)xz_removeFirstObject OBJC_SWIFT_UNAVAILABLE("请直接使用 Swift 版本");

/// 移除数组中指定位置的元素，并返回该元素。
/// - Parameter index: 元素的位置
